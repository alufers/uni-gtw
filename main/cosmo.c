...
#ifndef HOST_TEST
#include "webserver.h"   /* gtw_console_log */
#define COSMO_LOG(fmt, ...) gtw_console_log(fmt, ##__VA_ARGS__)
#else
#include <stdio.h>
 ...
 *
 *  [0..3]  KeeLoq encrypted block (big-endian 32-bit word).
...
esp_err_t cosmo_decode(const cosmo_raw_packet_t *raw, cosmo_packet_t *out)
{
    ...
    /* Decrypt the first 4 bytes (little-endian 32-bit word) */
