...
/**
 * @brief Decrypt and decode a raw CC1101 packet into a cosmo_packet_t.
 * @return ESP_OK on success, ESP_FAIL if command fields are inconsistent.
 */
esp_err_t cosmo_decode(const cosmo_raw_packet_t *raw, cosmo_packet_t *out);

/**
 * @brief Encode a cosmo_packet_t into a raw CC1101 packet ready for TX.
 */
esp_err_t cosmo_encode(const cosmo_packet_t *pkt, cosmo_raw_packet_t *out);

/**
 * @brief Format a decoded packet into a human-readable string.
...
/**
 * @brief Log a decoded packet via gtw_console_log (firmware only).
 */
void cosmo_packet_log(const cosmo_packet_t *pkt);

/**
 * @brief Return a string name for a cosmo_cmd_t value.
 */
